...
import { Table, Upload, Button, Typography, Tag, Icon, Form, Input } from 'antd';
...
class DesignCheckingList extends Component {
            ...
            .then(res => {
                ...
                if (res.status == 200) {
                    ...
                    res.data.forEach((booth, index) => {
                        const { boothId, boothName, auth } = booth
                        const status = auth.status || 'uncommited'
                        const reCheckButton = auth.status || 'uncommited'
                        const updatedAt = auth.updatedAt || ''
                        tableList.push({ boothId, boothName, updatedAt, reCheckButton, status, auth, key: index })
                    })
                    this.setState({
                        ...this.state,
                        tableList
    ...
    render() {
        ...
        const columns = [
            { title: '展位号', dataIndex: 'boothId', key: 'boothId' },
            { title: '展位名称', dataIndex: 'boothName', key: 'boothName' },
            { title: '提交时间', dataIndex: 'updatedAt', key: 'updatedAt' },
