...
const ExpoList = (props) => {
    ...
    const renderItem = (item) => {
        const joinedExpoList = JSON.parse(localStorage.getItem('expos'))
        let isAdded = false
        let btnText = '添加展会'
        for (let i = 0; i < joinedExpoList.length; i++) {
            if (joinedExpoList[i] === item.eid) {
                isAdded = true
                break
            }
        }
        ...
        return (
            ...
            >
                <List.Item.Meta
                    ...
                    title={<div>
                        <a href={item.eUrl} target='blank'><Text style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{item.eName}</Text></a>
                        <Button disabled={isAdded} onClick={(e) => onAddExpoClick(item.cid, e)} style={{ float: 'right', marginTop: '12px' }} type='primary' key={item.eid}>{btnText}</Button>
                    </div>}
                    description={<div>
                        <div>
